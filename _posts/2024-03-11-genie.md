...
1. 플레이어가 모델에 이미지 $x_1$을 넣어 프롬프팅 한다. (→ 첫번째 프레임으로 기능)
   - 이미지가 비디오 인코더를 통해 $z_1$으로 토큰화 된다. 
2. 플레이어가 잠재 액션 $a_1$([1, |A|) 사이의 정수)을 선택한다. 
    - Dynamics Model이 프레임 토큰 $z_1$과 대응되는 잠재 액션 $\tilde{a}\_1$ 을 받아 다음 프레임 토큰 $z_2$를 예측한다. 
3. 액션이 모델에 계속 입력됨에 따라 Autoregressive 방식으로 나머지 시퀀스 $\hat{z}\_{2:T}$를 생성한다. 
   - 동시에 토크나이저의 Decoder를 통해 토큰들이 비디오 프레임 $\hat{x}_{2:T}$로 Decode된다.
