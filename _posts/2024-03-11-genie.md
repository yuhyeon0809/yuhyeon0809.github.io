...
[ST-transformer](https://arxiv.org/pdf/2001.02908.pdf)는 Spatial-Temporal Transformer의 약자로, 공간적(Spatial) 정보와 시간적(Temporal) 정보를 함께 처리할 수 있는 트랜스포머 기반 모델입니다. 이미지 처리에 많이 쓰이는 [ViT(Vision Transformer)](https://arxiv.org/abs/2010.11929)와 달리 Temporal-attention 레이어가 있어 비디오 데이터 연산에 더욱 효과적인데요.

Genie에서 ViT가 아닌 ST-transformer를 채택한 이유 역시 이로부터 오는 **메모리 효율성** 때문입니다. 
