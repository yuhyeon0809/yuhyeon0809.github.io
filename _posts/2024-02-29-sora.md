...
Diffusion 모델의 Bockbone으로 U-Net이 아닌, Transformer 아키텍처를 적용한 모델이다. 줄여서 DiT라고 부르며, [ViT(Vision Transformer)](https://arxiv.org/abs/2010.11929)와 비슷하다. 이미지의 DDPM을 학습하는 것이 목적이며, 일련의 패치에서 작동하는 ViT를 기반으로 한다. 무엇보다 Transformer 아키텍처를 채택한 덕에 뛰어난 확장성을 지닌다는 것이 가장 큰 특징이다. [Diffusion Transformer 논문](https://arxiv.org/abs/2212.09748)을 참고하면 더 자세한 정보를 얻을 수 있다. 
